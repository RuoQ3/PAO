...
`.report.md` 是面向工程师的分析报告，不是 JSON 转储。它按“执行摘要—已完成工作—结果与工程含义—专业建议—结论边界”组织内容：

- 优化目标、预算使用、收敛/可行工况数量和本轮是否形成可交付结论；
- 已经完成的 Aspen 工况评估、定向参数试验和独立复验；
- 目标改善、产品约束瓶颈、代表工况和关键定向试验的工程含义；
- 下一轮应优先验证的参数方向、可行域修复建议和目标阈值建议。

报告解释的是已记录字段，不包含或声称包含 LLM 的隐藏思维链，也不把原始代码或 checkpoint 字段直接展示给
工程师。所有动作效果都标注为观察到的关联，不是单次实验的因果证明；`.report.json` 仍然是完整的机器可读
审计记录，适合复现和二次分析。

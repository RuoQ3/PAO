...
1. read_process_topology(driver)
   ...
   block_type 通过尝试读取 HAP_RECORDTYPE 属性获取；失败时读取节点 Value 降级。
...
def _get_block_type(driver: "AspenDriver", block_name: str) -> str:
    ...
    两级降级策略：
    1. 通过 AspenNode.info()（依赖 hap_constants）读取 HAP_RECORDTYPE
    2. hap_constants 不可用时直接读节点 Value（某些 Aspen 版本会返回类型字符串）
    """
    from src.aspen_driver.node import AspenNode
    path = f"{_BLOCKS_ROOT}\\{block_name}"
    try:
        node = AspenNode(driver, path)
        # info() 从 driver.hap_constants 获取 HAP 常量，无需传参
        info = node.info()
        return (info.record_type or "").upper()
    except Exception:
        # hap_constants 不可用时降级：直接读节点 Value
        try:
            val = driver.get_value(path)
...
def read_process_topology(driver: "AspenDriver") -> dict:
    ...
    for sname in stream_names:
        ...
        has_up   = bool(up_block   and up_block   in block_set)
        has_down = bool(down_block and down_block in block_set)
